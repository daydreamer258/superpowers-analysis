...
| 维度 | Superpowers TDD + Verification | OpenSpec Verify |
|------|-------------------------------|----------------|
| TDD 是否强制 | ✅ 铁律（违反要删代码） | ❌ 无内置 TDD |
| 验证是否强制 | ✅ 铁律（任何完成声明前） | ❌ 可选 command |
| 验证粒度 | 命令级证据（"run it, show output"） | 三维度结构化报告 |
| 谁触发 | agent 被规则约束自动触发 | 用户主动调用 |
| 是否阻止错误 | ✅ 阻止虚假完成声明 | ❌ 不阻止 archive |
| 借口防御 | ✅ Rationalization 表 | ❌ 无 |
