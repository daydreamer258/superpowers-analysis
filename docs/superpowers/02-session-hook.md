...
|------|------------|----------|
| 注入方式 | SessionStart hook 注入完整 skill 文本 | `openspec update` 生成静态文件 + CLI `--json` 动态查询 |
| 注入时机 | 每次会话启动（startup/clear/compact） | 文件一次生成 + 运行时按需查询 |
| 需要 CLI | ❌ 不需要 | ✅ 必须（`openspec instructions`） |
| 平台判断 | 环境变量（CLAUDE_PLUGIN_ROOT 等） | 25+ 适配器显式支持 |
| 内容量 | 大（整个 using-superpowers 一次性注入） | 小（模板+规则按 artifact 动态返回） |
| 自定义灵活性 | 写新的 Markdown skill | 写新的 YAML schema + profile |
